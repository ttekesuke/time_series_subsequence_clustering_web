...
3. **Search-based generation:** When provided with a target progression of complexity values, the system greedily proposes candidates step by step, measuring the complexity of each option from clustering results and choosing the sequences that best follow the requested trajectory.
...
- **Complexity-aware generation:** Estimates structural complexity from clustering results. Given a target progression of complexity values, the system greedily explores candidate sequences, scoring each by its clustering-derived complexity to keep pace with the requested trajectory.
