...
@testset "#22 short series has no phantom root and initializes when ready" begin
  ...
  @test isempty(IssuesPCM.logical_virtual_nodes(empty_mgr))
  @test isempty(IssuesPCM.clusters_to_timeline(empty_mgr))

  one_mgr = IssuesPCM.Manager(Vector{Float64}[[3.0]], 0.3, 2)
  IssuesPCM.process_data!(one_mgr)
  @test isempty(IssuesPCM.logical_virtual_nodes(one_mgr))
  IssuesPCM.add_data_point_permanently!(one_mgr, [4.0])
  one_rows = IssuesPCM.logical_virtual_nodes(one_mgr)
  @test length(one_rows) == 1
  @test one_rows[1].cluster_id == 0
  @test one_rows[1].si == [0]
  @test one_rows[1].as == [[3.0], [4.0]]

  normal_mgr = IssuesPCM.Manager(Vector{Float64}[[3.0], [4.0]], 0.3, 2)
  IssuesPCM.process_data!(normal_mgr)
  normal_rows = IssuesPCM.logical_virtual_nodes(normal_mgr)
  @test length(normal_rows) == 1
  @test normal_rows[1].cluster_id == 0
  @test normal_rows[1].si == [0]
end
